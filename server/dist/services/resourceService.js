...
export async function getResources(query) {
    const { search = '', type = 'all', dept = 'all', sem = 'all', subject = 'all', types, sort = 'latest', page = 1, limit = 8 } = query;
    ...
    let filtered = localResources.filter((r) => {
        if (type !== 'all' && r.type !== type)
            return false;
        if (dept !== 'all' && r.dept_id !== dept)
            return false;
        if (sem !== 'all' && r.semester !== sem)
            return false;
        if (subject !== 'all' && r.subject !== subject)
            return false;
        if (types) {
            const typeArray = Array.isArray(types) ? types : [types];
            if (typeArray.length > 0 && !typeArray.includes(r.type))
                return false;
        }
        if (search) {
...
export async function createResource(resourceData, file) {
    let fileUrl = '';
